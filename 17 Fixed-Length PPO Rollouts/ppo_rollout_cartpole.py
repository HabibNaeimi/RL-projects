...
def train(
    ...
    for update_index in range(1, num_updates + 1):
        ...
        if update_index % REPORT_EVERY == 0:
...
def plot_training_history(
        episode_returns,
        window=50,
        output_path="ppo_rollout_cartpole.png"
):
    ...
    plt.title("Fixed-Length PPO Rollouts and Boundary-Aware GAE")
    plt.ylim(0, 510)
    plt.grid(alpha=0.3)
    plt.legend()
    plt.tight_layout()
...
def main():
    ...
    episode_returns, episode_length  = train(
        env=env,
        policy=policy,
        value_network=value_network,
        actor_optimizer=actor_optimizer,
