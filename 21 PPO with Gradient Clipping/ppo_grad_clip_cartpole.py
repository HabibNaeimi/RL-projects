...
def get_gradient_norm(parameters):
    """
    Manually calculating pre-clipping norm.
    """
    ...
    for parameter in parameters:
        if parameter.grad is not None:
            ...
            flattened.append(grads.reshape(-1))

    if not flattened:
        return 0.0
    
    catted = torch.cat(flattened)
    norm = torch.linalg.vector_norm(catted, ord=2)
    return norm.item()
...
def clip_gradients(parameters, max_grad_norm):
    """
    Automatically calculating pre-clipping norm.
    """
    parameters = list(parameters)
    if max_grad_norm <= 0 or not math.isfinite(max_grad_norm):
        raise ValueError("The max_grad_norm must be positive and finite.")
...
def update_ppo_from_rollout(
    ...
    for update_epoch in range(update_epochs):
        ...
        for indices in minibatches:
            ...
            # Gradient Clipping
            # actor_grad_norm = get_gradient_norm(parameters=policy.parameters()) # Manually
            actor_pre_clip_norm = clip_gradients(    # Automatically
                parameters=policy.parameters(),
                max_grad_norm=max_grad_norm,
            )
            actor_grad_norms.append(actor_pre_clip_norm)
            actor_was_clipped.append(actor_pre_clip_norm > max_grad_norm)
            ...
            # Gradient Clipping
            # critic_grad_norm = get_gradient_norm(parameters=value_network.parameters()) # Manually
            critic_pre_clip_norm = clip_gradients(          # Automatically
                parameters=value_network.parameters(),
                max_grad_norm=max_grad_norm,
            )

            critic_grad_norms.append(critic_pre_clip_norm)
            critic_was_clipped.append(critic_pre_clip_norm > max_grad_norm)
    ...
    final_metrics = {
        ...
        "mean_actor_grad_norm": float(np.mean(actor_grad_norms)),
        "mean_critic_grad_norm": float(np.mean(critic_grad_norms)),
        "actor_grad_clip_fraction": float(np.mean(actor_was_clipped)),
        "critic_grad_clip_fraction": float(np.mean(critic_was_clipped)),
    }
...
def train(
    ...
    for update_index in range(1, num_updates + 1):
        ...
        if update_index % REPORT_EVERY_UPDATES == 0:
            ...
            print("mean_actor_grad_norm:", update_results["mean_actor_grad_norm"])
            print("mean_critic_grad_norm:", update_results["mean_critic_grad_norm"])
            print("actor_grad_clip_fraction:", update_results["actor_grad_clip_fraction"])
            print("critic_grad_clip_fraction:", update_results["critic_grad_clip_fraction"])     
...
def plot_training_history(
        episode_returns,
        window=50,
        output_path="ppo_grad_clip_cartpole.png"
):
    ...
    plt.title("PPO with Gradian Clippping")
    plt.ylim(0, 510)
    plt.grid(alpha=0.3)
    plt.legend()
    plt.tight_layout()
